...
def segment_frame_downscaled_ds(original_frame, model_path, out_path, ratio=0.2, diameter=30,is_frame_0=False):
    ...
    cellpose_model = models.CellposeModel(gpu=use_gpu, pretrained_model=model_path)
...
# Siamese Model for Reticulocyte Change Detection
...
six_class_model_path = str(MODELS_DIR / "best_model_vit_torch_macos_seven.pth")
...
# ========= 加载模型 =========
six_class_model = ViTClassifier(num_classes=7)
...
# Pairwise model for Class C (Reticulocyte) (changed vs unchanged)
pair_model_C = SiameseViTChange()
pair_model_C.load_state_dict(torch.load(str(MODELS_DIR / "OG1_siamese_vit_c_Haolin.pt"), map_location=device))
...
pair_model_F.load_state_dict(torch.load(str(MODELS_DIR / "siamese_vit_ISC_Haolin.pt"), map_location=device))
...
# General pairwise model for All classes
pair_model_All = SiameseViTChange()
pair_model_All.load_state_dict(torch.load(str(MODELS_DIR / "siamese_vit_All_Haolin.pt"), map_location=device))
    ...
    for frame_idx in tqdm(range(1, max_frame), desc="Processing frames"): # Note (Haolin): start from frame 1 to avoid overwriting frame 0 info
        ...
        for cid, info in matched.items():
            ...
            with torch.no_grad():
                ...
                # if cls_id == 3:  # ---- Reticulocyte uses the pairwise model ----
                #     # inputs
                #     ref_t = cell_info[cid]['ref_tensor'].to(device)   # (3,224,224)
                #     cur_t = transform(cell_pil).to(device)            # (3,224,224)
                ...
                #     # 5 is ISC
                #     # if cls_id == 6 and frame_idx>80:
                #     #     bin_output = binary_model_F(cell_tensor.to(device))
                #     #     bin_probs = 1 - torch.softmax(bin_output, dim=1)
                #     #     bin_label = torch.argmax(bin_probs, dim=1).item()
                #     #     bin_prob = bin_probs[0, bin_label].item()
                #     # elif cls_id == 6 and frame_idx < 80:
                #     #     bin_output = binary_model_Fb(cell_tensor.to(device))
                #     #     bin_probs = 1 - torch.softmax(bin_output, dim=1)
                #     #     bin_label = torch.argmax(bin_probs, dim=1).item()
                #     #     bin_prob = bin_probs[0, bin_label].item()
                #     if cls_id == 6: # ---- ISC uses the pairwise model ----
                #         # inputs
                #         ref_t = cell_info[cid]['ref_tensor'].to(device)   # (3,224,224)
                #         cur_t = transform(cell_pil).to(device)            # (3,224,224)
                ...
                #     elif cls_id == 4 and frame_idx<80:
                #         bin_output = binary_model_D(cell_tensor.to(device))
                #         bin_probs = 1 - torch.softmax(bin_output, dim=1)
                #         bin_label = torch.argmax(bin_probs, dim=1).item()
                #         bin_prob = bin_probs[0, bin_label].item()
