...
def segment_frame_downscaled_ds(original_frame, cellpose_model_path, out_path, ratio=0.2, diameter=30,is_frame_0=False):
    """
    ...
    # 加载模型并运行Cellpose
    cellpose_model = models.CellposeModel(gpu=False, pretrained_model=cellpose_model_path)
...
pair_model_C.load_state_dict(torch.load(str(MODELS_DIR / "OG_siamese_vit_c_Haolin.pt"), map_location=device))
...
def process_video(video_path, out_path, video_id, output_video_path,six_class_model,binary_model,feature_extractor,transform,cellpose_model_path = str(MODELS_DIR / "cyto3_train0327"),frame_skip=frame_skip,max_frame=max_frame,fps=fps):
    ...
    for cid in tqdm(unique_ids, desc='Process cells in frame 0 - progress:'):
        ...
        # cell_info[cid]['pair_above_hist'] = deque(maxlen=MIN_PERSIST)       # K-in-a-row buffer
        ...
        for cid, info in matched.items():
            ...
            with torch.no_grad():
                if cls_id == 2:  # ---- Class C uses the pairwise model ----
                    ...
                    # #MIN_PERSIST = 2     # consecutive frames above thr to flip
