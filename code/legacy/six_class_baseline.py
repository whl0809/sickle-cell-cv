...
def segment_frame_downscaled_ds(original_frame, model_path, out_path, ratio=0.2, diameter=30, is_frame_0=False):
    ...
    masks, flows, styles = cellpose_model.eval(resized_frame, diameter=30, channels=[0, 0])
...
def match_cells_tracking(prev_cells, curr_masks, bboxes):
    ...
    for track_id, prev_info in prev_cells.items():
        ...
        for cid in unmatched:
            ...
            if dist < min_dist or (dist < 50 and iou > best_iou):  # 结合距离与IoU
                min_dist = dist
                best_iou = iou
                best_cid = cid
                best_box = curr_box
...
def process_video(video_path, out_path, video_id, output_video_path,six_class_model,binary_model,feature_extractor,transform,cellpose_model_path = str(MODELS_DIR / "cyto3_train0327"),frame_skip=frame_skip,max_frame=max_frame,fps=fps):
    ...
    for cls_id in range(num_classes):
        ...
        #plt.plot(df['FrameIndex'], df[f'Class_{cls_id}'], label=f'Class {cls_id}')
        plt.plot(time_sec, y_percent, label=f'{dname[cls_id]} ({count}')
...
for idx, video_path in enumerate(video_paths):
    ...
    # debug_remove_edge_masks(out_path[idx])
    ...
    #cell_info1,df1=process_video(video_path, video_id=f"V{idx+1}", output_video_path=output[idx],six_class_model,binary_model,feature_extractor,transform,cellpose_model_path = str(MODELS_DIR / "cyto3_train0327"))
