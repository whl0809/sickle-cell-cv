...
def segment_frame_downscaled_ds(original_frame, cellpose_model, out_path, ratio=0.2, diameter=30,is_frame_0=False):
    """
...
six_class_model_path = str(MODELS_DIR / "best_model_vit_torch_macos.pth")
binary_model_path = str(MODELS_DIR / "best_model_vit_torch_macos_raw_vit_large_binary.pth")
binary_model_path_C = str(MODELS_DIR / "direct_vit_C.pt")
binary_model_path_D = str(MODELS_DIR / "direct_vit_D.pt")
binary_model_path_F = str(MODELS_DIR / "best_model_vit_torch_macos_raw_vit_large_binary_F.pth")
binary_model_path_F_brandon = str(MODELS_DIR / "direct_vit_F.pt")
...
cellpose_model_path = str(MODELS_DIR / "cyto3_train0327")
cellpose_model_used = models.CellposeModel(device=device if use_gpu else torch.device("cpu"), gpu=use_gpu, pretrained_model=cellpose_model_path)
...
# Pairwise model for Class C (changed vs unchanged)
pair_model_C = SiameseViTChange()
pair_model_C.load_state_dict(torch.load(str(MODELS_DIR / "siamese_vit_c_Haolin.pt"), map_location=device))
...
#exit()
def process_video(video_path, out_path, video_id, output_video_path,six_class_model,binary_model,feature_extractor,transform,cellpose_model_path = str(MODELS_DIR / "cyto3_train0327"),frame_skip=frame_skip,max_frame=max_frame,fps=fps):
    # ========= 视频初始化 =========
    ...
    bboxes, masks_seg, unique_ids, resized_frame = segment_frame_downscaled_ds( first_frame, cellpose_model_used, out_path, is_frame_0=True)
    ...
    for cid in tqdm(unique_ids, desc='Process cells in frame 0 - progress:'):
        ...
        # --- NEW: init pairwise model state (must come AFTER cell_info[cid] is created) ---
        ...
        cell_info[cid]['pair_above_hist'] = deque(maxlen=3)       # K-in-a-row buffer
        cell_info[cid]['latched_changed'] = False                 # one-way latch
        # --- END NEW ---
    ...
    for frame_idx in tqdm(range(1, max_frame), desc="Processing frames"): # Note (Haolin): start from frame 1 to avoid overwriting frame 0 info
        ...
        bboxes, masks, unique_ids, resized_frame = segment_frame_downscaled_ds(frame, cellpose_model_used, out_path,ratio=0.1, diameter=30)
        #continue
        #exit()
        #print(masks)
        ...
        for cid, info in matched.items():
            ...
            with torch.no_grad():
                if cls_id == 2:  # ---- Class C uses the pairwise model ----
                    ...
                    # Smooth + latch to reduce flicker
                    prev = cell_info[cid]['pair_score_ema']
                    s_ema = p_changed if prev is None else (0.2 * p_changed + 0.8 * prev)
                    cell_info[cid]['pair_score_ema'] = s_ema

                    thr = 0.7           # tune on your validation videos if needed
                    MIN_PERSIST = 3     # consecutive frames above thr to flip
                    hist = cell_info[cid]['pair_above_hist']
                    hist.append(s_ema >= thr)
                    if (not cell_info[cid]['latched_changed']) and all(hist):
                        cell_info[cid]['latched_changed'] = True

                    #bin_label = LABEL_CHANGED if cell_info[cid]['latched_changed'] else LABEL_UNCHANGED # Enable latch and K-in-a-row                    
                    #bin_label = LABEL_CHANGED if all(hist) else LABEL_UNCHANGED # No latch, only K-in-a-row
                    bin_label = LABEL_CHANGED if s_ema >= thr else LABEL_UNCHANGED # No latch, no K-in-a-row, only thr of EMA
                    bin_prob  = float(s_ema)  # for logging/plots
